...
mvViewScore    = log10(musicVideoViews + 1) × 12`} description="토픽 채널 조회수/구독자 및 MV 전용 조회수는 log10을 통해 전체 music_score에 가중 반영됩니다." />
      ...
      <p className="text-xs text-muted-foreground">7개 소스의 가중 멘션을 집계합니다. Firecrawl Search로 X, Reddit, TikTok, News를 크롤링하고, YouTube 댓글과 네이버 뉴스는 별도 수집합니다.</p>
...
sentimentBonus = (sentimentScore − 50) × 4`} />
      <Card className="p-3 bg-card border-border/50">
        <p className="text-[11px] text-muted-foreground font-medium mb-1">소스별 가중치 (7개 소스)</p>
        <code className="block text-[11px] font-mono text-primary bg-primary/5 rounded px-2 py-1.5 whitespace-pre-wrap">{`News:             2.0x  ← 가장 높은 시그널 품질
...
External Videos:  1.2x  ← 외부 채널 출연`}</code>
        <p className="text-[10px] text-muted-foreground mt-1.5">네이버 뉴스 수집이 Buzz보다 먼저 실행되어 한국 미디어 데이터가 집계에 포함됩니다.</p>
      </Card>

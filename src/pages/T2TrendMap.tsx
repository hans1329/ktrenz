...
const T2TrendMap = () => {
  ...
  return (
    <>
      ...
      {/* Title + Sort — collapses on scroll */}
      <div
        className="fixed top-14 left-0 right-0 z-40 transition-all duration-300 overflow-hidden"
        style={{
      ...
      >
        ...
        >
          {VIEW_ORDER.map((mode) => (
            <div key={mode} className={cn("h-full w-full flex-shrink-0 overflow-y-auto overscroll-contain pb-24 scrollbar-hide transition-all duration-300", headerCollapsed ? "pt-[3.25rem]" : "pt-[9.5rem]")}
              onScroll={(e) => {

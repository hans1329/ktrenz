...
import { TrendingUp, ChevronUp, ChevronDown, ChevronRight, Flame, LayoutGrid, List, Crown, Medal, Youtube, Twitter, Music, Disc3, Loader2, Star, Zap, MoreVertical } from "lucide-react";
...
const V3TrendRankings = () => {
  ...
  const toInspectorItem = useCallback((item: any): InspectorItem => {
    ...
    return {
      ...
      socialChange24h: Number(item.social_change_24h ?? 0),
  ...
  const { data: rankings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: allScores, error } = await supabase
        ...
        .select(`wiki_entry_id, youtube_score, total_score, energy_score, energy_change_24h, buzz_score, album_sales_score, music_score, fan_score, social_score,
          youtube_change_24h, buzz_change_24h, album_change_24h, music_change_24h, fan_change_24h, social_change_24h, scored_at,

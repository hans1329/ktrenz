...
import { TrendingUp, Bot, Power, Activity } from "lucide-react";
...
export type V3Tab = "rankings" | "agent" | "activity";
      ...
      <nav className="fixed bottom-2 left-3 right-3 z-50 bg-background/95 backdrop-blur-md border border-purple-500/30 rounded-full overflow-visible"
        ...
        <div className="flex items-center justify-evenly h-16 max-w-md mx-auto">
          {tabs.map((tab) => {
            ...
            return (
              <button key={tab.id} onClick={() => (tab.id === "agent" ? navigate("/agent") : tab.id === "activity" ? navigate("/dashboard") : onTabChange(tab.id as V3Tab))}

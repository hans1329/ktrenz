  ...
  return (
    <>
      <button
        ...
        className="flex items-center px-0.5 py-1 active:opacity-60 transition-opacity"
      >
        <div className={cn(
          "relative w-16 h-5 rounded-full bg-muted overflow-hidden",
          isFull && "animate-[pulse_2s_ease-in-out_infinite]",
        )}>
          {/* Base text: dark on unfilled muted bg. Push centerline right to
              avoid overlapping the icon at left. */}
          <span className="absolute inset-0 flex items-center justify-center pl-4 text-[9px] font-bold leading-none tracking-tight text-muted-foreground">
            {Math.round(progress)}%
          </span>
          ...
          >
            <span className="absolute top-0 left-0 h-full w-16 flex items-center justify-center pl-4 text-[9px] font-bold leading-none tracking-tight text-white">
              {Math.round(progress)}%
            </span>
          </div>
          {/* Spotify icon — overlaid on the left, white so it's legible
              against both the muted track and the green fill. */}
          <div className="absolute left-1 top-1/2 -translate-y-1/2 w-3.5 h-3.5 pointer-events-none">

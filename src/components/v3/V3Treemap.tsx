...
function squarify(items: TreemapItem[], x: number, y: number, w: number, h: number, category: EnergyCategory, isCollecting = false): Rect[] {
  ...
  const tileSize = (i: TreemapItem, idx: number) => {
    ...
    if (isCollecting) {
      if (idx === 0) return base * 2.4;
      if (idx === 1) return base * 2.15;
      if (idx === 2) return base * 1.95;
      if (idx === 3) return base * 1.8;
      if (idx === 4) return base * 1.65;
      if (idx === lastIdx) return base * 1.8;
      return base * 1.3;
    }
          ...
          {rects.map((rect, rectIndex) => {
            ...
            return (
              <button key={rect.item.id} onClick={() => handleTileClick(rect.item)}
                ...
                {rect.item.isNew && (
                  <span className="absolute top-0 right-0 z-30 bg-amber-400 text-white font-black rounded-bl-md flex items-center justify-center"

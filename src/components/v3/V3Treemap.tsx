  ...
  return (
    <Drawer open onOpenChange={(open) => { if (!open) onClose(); }} handleOnly>
      <DrawerContent className={cn(
        "max-h-[75vh] lg:max-h-[85vh] rounded-t-2xl border-t mx-auto max-w-[600px] bg-card flex flex-col",
              ...
              <button key={rect.item.id} onClick={() => handleTileClick(rect.item)}
                ...
                {isFirst && (
                  <div className="absolute inset-0 z-[1] pointer-events-none" style={{
                    boxShadow: 'inset 0 0 40px 12px hsla(45, 100%, 70%, 0.7), inset 0 0 80px 25px hsla(38, 100%, 60%, 0.4), inset 0 0 120px 40px hsla(30, 100%, 50%, 0.2)',
                    background: 'radial-gradient(ellipse at center, hsla(45, 100%, 80%, 0.15) 0%, transparent 70%)',
                  }} />
                )}

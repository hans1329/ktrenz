...
const T2DetailSheet = ({ tile, rank, totalCount, onClose }: { tile: TrendTile | null; rank?: number; totalCount?: number; onClose: () => void }) => {
  ...
  const handleSubmitPrediction = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["hero-bet-stats"] });
      queryClient.invalidateQueries({ queryKey: ["hero-tickets"] });
      refetchTickets();
  ...
  return (
    <Sheet open={!!tile} onOpenChange={(open) => { if (!open) { onClose(); } }}>
      <SheetContent side="bottom" className="rounded-t-2xl max-h-[90dvh] overflow-hidden border-border sm:max-w-lg sm:left-1/2 sm:-translate-x-1/2 flex flex-col gap-0 bg-background">

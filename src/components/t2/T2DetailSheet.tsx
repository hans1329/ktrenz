...
const T2DetailSheet = ({ tile, rank, totalCount, onClose }: { tile: TrendTile | null; rank?: number; totalCount?: number; onClose: () => void }) => {
  ...
  return (
    <Sheet open={!!tile} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="bottom" className="rounded-t-2xl max-h-[85dvh] overflow-hidden border-border sm:max-w-lg sm:mx-auto flex flex-col bg-background">
        <SheetHeader className="pb-0 shrink-0 -mx-6 px-6 -mt-6 pt-6 rounded-t-2xl">

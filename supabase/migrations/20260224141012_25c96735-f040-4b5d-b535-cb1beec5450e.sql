-- Generated column은 BEFORE UPDATE 트리거에서 NEW에 반영되지 않을 수 있음
-- 트리거를 수정: NEW.total_score 대신 직접 계산
...
DECLARE
  ...
  old_total numeric;
BEGIN
  -- generated column이 BEFORE 트리거에서 반영 안 되므로 직접 계산
  ...
  IF TG_OP = 'UPDATE' AND old_total > 0 THEN
    velocity := ABS(calculated_total - old_total) / old_total * 100;
    IF calculated_total > old_total THEN

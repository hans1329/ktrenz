...
BEGIN
  SELECT COALESCE(
    (SELECT CASE WHEN ks.tier_id = 3 THEN 'pro' WHEN ks.tier_id = 2 THEN 'basic' ELSE 'free' END
     FROM kpass_subscriptions ks WHERE ks.user_id = _user_id AND ks.status = 'active'
  ...
  FROM ktrenz_agent_daily_usage WHERE user_id = _user_id AND usage_date = _today;
  ...
  IF _used < _limit THEN
    ...
    DO UPDATE SET message_count = ktrenz_agent_daily_usage.message_count + 1, updated_at = now();
    RETURN jsonb_build_object('allowed', true, 'used', _used + 1, 'daily_limit', _limit, 'tier', _tier, 'point_used', false);
  END IF;

  SELECT COALESCE(SUM(CASE WHEN type = 'earn' THEN amount ELSE -amount END), 0) INTO _points
  FROM ktrenz_point_transactions WHERE user_id = _user_id;

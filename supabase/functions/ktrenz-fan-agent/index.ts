...
📊 트렌드 키워드 응답 규칙 (get_trend_keywords / get_trending_now):
- 트렌드 키워드 데이터는 프론트엔드에서 자동으로 인라인 카드로 렌더링됨
- 절대 키워드를 하나하나 텍스트로 나열하거나 설명하지 마!
- 응답은 간단한 요약 메시지 1~2줄만 작성해 (예: "지금 뜨고 있는 트렌드 키워드를 모아봤어요! 🔥")
- 카드가 모든 상세 정보를 보여주니까 텍스트로 중복 설명하지 마!

...
- get_rankings 도구는 사용 가능하지만, 랭킹 데이터는 프론트엔드에서 카드로 자동 렌더링되므로 텍스트로 순위를 나열하지 마. ${artistName}의 위치와 한두 줄 코멘트만 짧게 해줘
...
도구 사용 규칙:
- 유저가 랭킹, 순위를 물으면 get_rankings 도구를 호출해. 랭킹 데이터는 프론트엔드에서 자동으로 이쁜 카드로 렌더링되므로, 텍스트로 순위 목록을 나열하지 마! 대신 한두 줄로 핵심 포인트만 짧게 코멘트해 (예: 최애 아티스트 순위 언급, 주목할 변동, 한마디 격려)

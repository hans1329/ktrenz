...
async function detectForMember(
  ...
  if (!lightMode && bodyFetchTargets.length > 0) {
    await Promise.allSettled(bodyFetchTargets.map(async (article) => {
      try {
        ...
        article.bodyExcerpt = bodyText.slice(0, 500);
  ...
  console.log(`[trend-detect] ${member.display_name}: news=${selectedNews.length}/${filteredNews.length}(total=${newsResult.total}) blog=${sibFilteredBlogs.length}(total=${blogResult.total}) youtube=${filteredYT.length}(total=${ytResult.totalResults}) articles(deduped)=${articles.length} shop=${shopItems.length} shopKW=${shopKeywords.length} fashionMedia=${fashionMediaItems.length} fashionNews=${fashionNewsItems.length}`);
  ...
  for (const candidate of candidateRows) {
    ...
    // 순수 인물명 필터 (한글 2~4자 이름만으로 구성된 키워드 제거, 복합 키워드는 유지)
    // 단, 상업적 카테고리(brand, product, fashion, beauty, food, restaurant)는 예외
    const commercialCategories = new Set(["brand", "product", "fashion", "beauty", "food", "restaurant", "shopping"]);
    const candidateCategory = (candidate.keywordRow.keyword_category || "").toLowerCase();
    if (!commercialCategories.has(candidateCategory)) {
